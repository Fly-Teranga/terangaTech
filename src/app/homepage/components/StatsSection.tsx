...
export default function StatsSection() {
  ...
  return (
    <section ref={sectionRef} className="border-black/6 bg-white px-6 py-24 border-y">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {differentiators.map((item, i) => (
            <div key={item.title} className="reveal-scale bg-light-bg hover:shadow-card group rounded-2xl border border-black/6 p-7 transition-all duration-300 hover:border-vert/30" style={{ transitionDelay: `${i * 100}ms` }}>
              <div className="mb-4 inline-block origin-left text-2xl transition-transform group-hover:scale-110">{item.icon}</div>
              <h3 className="font-display text-text-dark mb-3 text-base font-bold">{item.title}</h3>
              <p className="text-text-muted text-sm leading-7 font-light">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
